...
const getStats = () => {
  ...
  const avgSalary =
    employees.reduce((acc, emp) => acc + (parseFloat(emp.salary) || 0), 0) /
    (totalEmployees || 1);
  ...
  return [
    {
      ...
      icon: "users",
      color: "#154295",
      trend: "+" + (totalEmployees > 5 ? totalEmployees - 5 : 0),
      trendUp: true,
    },
    {
      label: "Avg Salary",
    ...
    },
    {
      ...
      icon: "shield-check",
      color: "#2e7d32",
      trend: Math.round((activeCount / totalEmployees) * 100) + "%",
      trendUp: true,
    },
    {
      label: "System Status",
    ...
    .map(
      (emp) => `
        <tr class="animate-fade">
            <td>
                <div class="emp-info">
                    <div class="dev-avatar" style="width: 32px; height: 32px; font-size: 0.8rem; margin-bottom: 0;">${emp.name
                      .split(" ")
                      .map((n) => n[0])
                      .join("")}</div>
                    <div>
...
function createWelcomePage() {
  ...
  welcome.style.cssText = `
    ...
    background: radial-gradient(circle at top left, rgba(21,66,149,0.18), transparent 30%), rgba(6,11,26,0.98);
    color: #ffffff;
    z-index: 2000;
    padding: 1.5rem;
  `;
  welcome.innerHTML = `
    <div style="width: min(560px, 100%); border-radius: 1.5rem; text-align: center;">
      <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 1.5rem;">
        <img src="public/ddulogo.png" alt="DDU Logo" style="width: 150px; height: auto; border-radius: 12px;" />
      </div>
      <div style="margin-bottom: 1.5rem;">
        <h1 style="font-size:2.4rem; margin:0; line-height:1.1;">Dire Dawa University</h1>
        <p style="color:#cbd5e1; margin:0.5rem 0 0; font-size:1rem;">Employee Management Portal</p>
      </div>
      <p style="color:#d1d5db; margin-bottom:2rem; font-size:1rem; line-height:1.7;">A secure admin experience for managing faculty, departments, payroll and staff records. Click Get Started to proceed to the admin login page.</p>
      <button id="get-started-btn" style="background: linear-gradient(135deg, #154295, #fbb03b); border: none; color: #111827; padding: 1rem 2rem; font-size: 1rem; border-radius: 999px; cursor: pointer; font-weight: 700; transition: transform 0.2s ease, opacity 0.2s ease;">Get Started</button>
    </div>
  `;

  document.body.prepend(welcome);
  const button = welcome.querySelector("#get-started-btn");
  button?.addEventListener("click", () => {
    welcome.style.opacity = "0";
    welcome.style.transition = "opacity 180ms ease";
    setTimeout(() => {
      welcome.remove();
...
const renderPayroll = () => {
  ...
  const totalPayroll = employees.reduce(
    (acc, e) => acc + (parseFloat(e.salary) || 0),
    0,
  );
...
async function navigate(viewName, filter = null) {
  ...
  if (viewName === "employees") {
    ...
    document
      ...
      ?.addEventListener("click", () => navigate("employees"));
  ...
  if (viewName === "departments") {
    document
      .getElementById("add-dept-as-staff")
  ...
  if (viewName === "payroll") {
    ...
    document
      .getElementById("refresh-payroll")
      ?.addEventListener("click", () => navigate("payroll"));
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (logoContainer) {
    logoContainer.innerHTML = `<img src="public/ddulogo.png" alt="DDU Logo" style="width: 40px; height: auto;">`;
  }
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (username === "admin" && password === "admin123") {
        ...
        alert(
          "Connection to backend failed. Please ensure your PHP server is running.",
        );
      }
    }
  });
